...
### 2026-04-20 — v4.4.1 GitHub release + release infrastructure

- **Release**: [`v4.4.1`](https://github.com/ehrlinger/hazard/releases/tag/v4.4.1) tagged at `8379d64` with 4 platform archives (linux-x64, linux-arm64, macos-arm64, windows-x64) + `SHA256SUMS.txt`.
- **Historical tag**: [`v4.3.1`](https://github.com/ehrlinger/hazard/releases/tag/v4.3.1) at 2016 commit `bfa525c`, source-only (release.yml didn't exist at that commit).
- **PR [#20](https://github.com/ehrlinger/hazard/pull/20)** (merged `5c3341a`): drop `macos-x64` from `release.yml` matrix (`macos-13` runner retired from GitHub Actions free tier); document source-build guidance for Intel Mac in `INSTALL.md` and `docs/vignettes/downloads.md`.
- **v4.4.1 release assembly**: initial tag-triggered build stuck on `macos-x64` queue for 40+ min; cancelled the run, downloaded the 4 successful artifacts from the matrix, assembled the release manually via `gh release create`. Future tag pushes will use the PR #20 workflow and build cleanly.

### 2026-04-20 — v4.4.2 V2 validator hotfix (release/4.4)

- **Release**: `v4.4.2` on `release/4.4` branch. **Note**: `release/4.4` branch cut from `v4.4.1` during this work, reversing the earlier "not pre-emptively" decision — having the maintenance line in place simplified the hotfix flow.
- **PR [#21](https://github.com/ehrlinger/hazard/pull/21)** (merged to `release/4.4`, commits `b8aa0b0` + `5dccde8`):
  - Root cause (corrected from the v4.4.1 log): reference `.lst` files have CRLF line endings; `awk '{print $NF}'` emits the last field with trailing `\r`; that CR got interpolated into a `python3 -c "..."` single-quoted source string, causing a Python `SyntaxError` before any comparison ran. The old log's "flags identical values as mismatches" framing was the visible symptom, not the cause — the comparator was never executing.
  - `strip_cr()` helper (`tr -d '\r'`) added to all four extractors.
  - `numbers_equal_dp` / `numbers_within_pct` take values via `sys.argv` + `.strip()` from a heredoc instead of string-interpolated Python (defence in depth — any CR becomes strippable whitespace, not a syntax break).
  - Per Copilot review: `tr` added to `preflight_dependencies`; three `head -1` extractors switched to `grep -m1` (consistency with `validate_numerical.sh`, no SIGPIPE under `pipefail`); `extract_final_loglik` kept `tail -1` (needs last match).
- **PR [#22](https://github.com/ehrlinger/hazard/pull/22)** (merged to `main`, merge commit `ce15f6a`): cherry-picks the validator fix only (no version bump) so v5.0 development on main has a working V2 gate.
- **Validation**: V2 smoke-test 20/20 PASS on both `release/4.4` and `main` after the fix.
- **Next session**: **corpus expansion decision pending SAS access.** Blockers and options captured below.

### Open: `tests/4.4.x/` reference-corpus expansion (blocked on tooling choice)

The roadmap §1 calls for a full reference corpus under `tests/4.4.0/` generated by running v4.4.x against every `.sas` example. Reality check done 2026-04-20:

- 23 `.sas` scripts under `tests/`, 19 committed `.lst` files, no `tests/inputs/` directory.
- The pipeline is **SAS-driven**: `.sas` → SAS reads flat data from `examples/data/` → converts to XPORT → C binary reads XPORT + PROC options via stdin. The C binary doesn't consume `.sas`.
- Without SAS we cannot regenerate `.input` files for the 15-16 examples not already covered by the validators. The 4 covered examples (`hz.death.AVC`, `hm.death.AVC`, `hz.deadp.KUL`, `hz.te123.OMC`) we know v4.4.2 reproduces bit-exactly.

**Options on the table (decision deferred pending SAS-access confirmation):**

1. **Freeze committed `.lst` as `tests/4.4.2/` baseline.** Zero-SAS, fastest; accepts that the initial snapshot represents "whatever produced these files historically," not verified v4.4.2 output. Validators flip to the new reference; regressions caught going forward.
2. **Write a SAS-free XPORT input harness.** Engineering-heavy (XPORT encoder, flat-file parser per-example); genuine reproducibility; multi-day.
3. **Partial expansion.** Hand-craft `.input` files for the examples with simple flat-file inputs — 3-4 more beyond the existing 4, gets us to ~7-8/20 coverage.
4. **Defer.** Move straight to Phase 5.1 (math-neutral; existing 4-example validators cover it); revisit corpus expansion when SAS is available.

**If SAS access lands by 2026-04-21**: use it to generate the full corpus + commit as `tests/4.4.2/`. This is the roadmap's intended plan.

**If SAS access does not land**: default to option 1 (freeze snapshot) + option 3 in parallel (hand-craft a few more inputs). Skip option 2. Then begin Phase 5.1.

**Decision owner**: @ehrlinger, checking SAS access next session.

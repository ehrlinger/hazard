...
**Context:** an afternoon of debugging the v4.4.5 Linux + SAS reference capture on the CCF Linux SAS host (`lri-sas-p-02.lerner.ccf.org`). Captures completed on Edwards (Windows + SAS); the same workflow fails on Linux SAS in a way that is **silent at every layer** until the failure cascades into a downstream parser explosion.
...
- **Edwards (Windows + SAS) does not exhibit this** — same bootstrap macros, same example drivers, runs in 7 seconds with `exit=0`.

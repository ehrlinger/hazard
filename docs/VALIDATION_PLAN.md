...
| # | Decision | Rationale |
|---|---|---|
| D1 | **v4.3.1 is the sole reference.**  No parallel v4.4.2 baseline. | The modern binary should match v4.3.1 bit-exactly (no numerical changes shipped in v4.4.x).  Any divergence is a regression to investigate, not an acceptable drift. |
...
| D7 | **Ship the scaffolded harness as v4.4.3** on `release/4.4`. | Release statement = "v4.4.3 reproduces v4.3.1 bit-exactly on the full 23-example corpus."  Tag anchors the parity claim; future comparisons diff the harness against the v4.4.3 baseline. |

...
> Created: 2026-04-21
> Status: landed in v4.4.3 — acceptance harness + two-bucket reference corpus (`v4.3.0` gcc + `v4.4.4-macos-arm64` clang-apple) checked in, CCF re-capture of 2026-04-23 complete. One open item: v4.4.x Linux recapture to retire the cosmetic banner/org-string diff against `v4.3.0` (see `tests/corpus/FINDINGS.md` §5 item 5).
...
| # | Decision | Rationale |
|---|---|---|
| D1 | **Two reference sets currently checked in, keyed to toolchain family: `v4.3.0` (archived CCF Linux capture — gcc bucket) and `v4.4.4-macos-arm64` (self-consistency baseline on Apple Silicon — clang-apple bucket).**  The v4.3 → v4.4.x numerical divergence between buckets is a cross-toolchain floating-point effect, not a code-history regression — confirmed via one-shot GHA workflows that produced matching LLs on Linux gcc across v4.3.1 and v4.4.2 (see `tests/corpus/FINDINGS.md` §2a).  The harness auto-selects by host: Darwin/arm64 → `v4.4.4-macos-arm64` (trivially green until someone modifies a numerical code path); Linux/Windows → `v4.3.0` (CCF ground truth, matches v4.4.x numerically but diffs on the version banner + org string pending a v4.4.x Linux recapture).  `REFERENCE=v4.3.0` on macOS surfaces the cross-toolchain drift for audit. | Keep the plan honest about what actually shipped and which toolchain each reference came from.  Single-bucket collapse isn't possible without compiler-level FP intervention (out of scope for v4.4). |

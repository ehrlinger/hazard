...
> **Host and paths are redacted by default.**  The wrapper writes `host=$(uname -s -r -m)`, `pwd=<redacted>` and a bare `real_bin` basename into each `.meta`, so no site hostname or `$HOME` path ends up in the corpus that ships back.  This corpus is committed to a **public** repository, so keep it that way.  Export `HAZARD_CAPTURE_REDACT=1` before the SAS loop — the wrapper will emit `uname -s -m -r` and `pwd=<redacted>` instead, keeping only OS/kernel + binary-hash provenance.

...
> **Optional — redact host/pwd from captured metadata.**  By default the wrapper writes `host=$(uname -a)` and `pwd=$(pwd)` into each `.meta` file, so `<redacted-host>` and your full `$HOME` path end up in the corpus that ships back.  If that's not appropriate for your site, export `HAZARD_CAPTURE_REDACT=1` before the SAS loop — the wrapper will emit `uname -s -m -r` and `pwd=<redacted>` instead, keeping only OS/kernel + binary-hash provenance.

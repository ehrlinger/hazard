...
#      Windows-host capture-windows-outputs.ps1) to produce a fresh
...
# Examples
# --------
#   # Diff a fresh Windows host capture against the v4.4.5-windows-x64-sas
...
if [[ ! -d "$ref_dir" ]]; then
    ...
    echo "       (Linux / production CCF) or tests/capture-windows-outputs.ps1 (Windows SAS host)." >&2
    exit 2
fi

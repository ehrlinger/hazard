...
rem Change the temporary file location for the tests.
rem
rem This way we can hold the temp files and compare them as well
rem export TMPHOLDER=$TMPDIR
rem export TMPDIR=`pwd`

echo temp storage in %TMPDIR%

rem # Start running the tests.
rem # Change this to a for loop (foreach .sas FILE, run and compare.)
